...
export function createRouter(
	...
	router.use((req, res, next) => {
		log(4, req.requestId, `${req.method} call to ${req.originalUrl}`)
		next()
	})
	...
	if (useCatchAll) {
		router.use((req, res) => {
			log(3, req.requestId, `Call to unknown path: ${req.path}`)
...
function createSubrouter(config, log, validate, router = Router({ mergeParams: true })) {
	...
	if (middleware) {
		if (!middleware.forEach || middleware.some(mw => typeof mw !== 'function')) {
			log(1, 0, 'Invalid router config. Middleware needs to be an array of functions.')
...
function createEndpoint(router, method, config, log, validate) {
	...
	if (!controller || typeof controller !== 'function') {
		log(1, 0, 'Invalid router config. Endpoints need to define a controller function.')
